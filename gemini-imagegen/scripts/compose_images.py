...
Usage:
    ./compose_images.py "instruction" output.jpg image1.jpg [image2.jpg ...]

Examples:
    ./compose_images.py "Create a group photo of these people" group.jpg person1.jpg person2.jpg
    ./compose_images.py "Put the cat from the first image on the couch from the second" result.jpg cat.jpg couch.jpg
    ./compose_images.py "Apply the art style from the first image to the scene in the second" styled.jpg style.jpg photo.jpg

Note: Supports up to 14 reference images (Gemini 3 Pro only).

Environment:
    NANO_BANANA_API_KEY or GEMINI_API_KEY - Required API key
"""
...
def get_api_key() -> str:
    ...
    if not api_key:
        raise EnvironmentError("NANO_BANANA_API_KEY or GEMINI_API_KEY not set")
    return api_key
