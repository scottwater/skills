...
**Setup:** Requires `NANO_BANANA_API_KEY` environment variable (or `GEMINI_API_KEY`). Scripts are self-contained and use `uv` for dependency management - no setup needed.
...
client = genai.Client(api_key=os.environ["NANO_BANANA_API_KEY"])
...
If you specifically need PNG format, use PIL to convert from the raw bytes:
...
for part in response.parts:
    if part.inline_data:
        # Get raw bytes and convert with PIL
        pil_img = Image.open(io.BytesIO(part.inline_data.data))
        pil_img.save("output.png", format="PNG")
```

**Note:** `part.as_image()` returns a google-genai image object whose `save()` method doesn't accept a `format` parameter. Use `part.inline_data.data` to get the raw bytes for PIL conversion.
