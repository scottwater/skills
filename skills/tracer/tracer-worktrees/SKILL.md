...
description: Set up an isolated workspace for feature work — detect existing isolation, prefer the harness's native worktree tool, fall back to git worktree. Use before running /tracer-implement on tickets in parallel.
disable-model-invocation: true
---

# Worktrees

Ensure work happens in an isolated workspace. This is what makes parallel `/tracer-implement` runs safe: each ticket gets its own checkout, branch, and `.tracer/implement/` workspace, and every task commits, so nothing rides on a shared dirty tree.
