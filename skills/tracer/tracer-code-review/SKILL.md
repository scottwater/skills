...
description: Standards-versus-Spec review. Use when the user requests Tracer's two-axis verdicts or a workflow assigns this review.
---
...
Whatever the user said is the fixed point — a commit SHA, branch name, tag, `main`, `HEAD~5`, etc. If they didn't specify one, ask for it. A workflow caller supplies its own fixed point and review budget.
...
**Workflow-owned review:** return after step 5. The caller owns adjudication, repair, and the stopping rule. Any further review requires an assignment within its budget.

**Standalone self-review:** a review with confirmed open Critical/Important findings means **the work is not done** — reporting is not resolving. If you produced the diff and no enclosing workflow owns its repair budget:
