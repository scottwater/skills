...
- **Issue tracker** — where `/to-spec` and `/to-tickets` publish and `/code-review` fetches specs from (GitHub, GitLab, local markdown, or a workflow you describe)
...
Tell the user which skills now read these files (`/to-spec`, `/to-tickets`, `/code-review`, `/implement`; all skills for domain docs) and that editing `docs/agents/*.md` directly is the normal way to adjust later — re-running this skill is only for switching trackers.
