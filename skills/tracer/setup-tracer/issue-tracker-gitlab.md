...
- **Merge requests**: GitLab calls PRs "merge requests" — `glab mr create`, `glab mr view`, `glab mr note`, the same shape as `gh pr ...` with `mr` for `pr` and `note`/`--message` for `comment`/`--body`. (`/finish-branch`'s "create a PR" means `glab mr create` here.)
...
Create a GitLab issue. Tickets published by `/to-tickets` get the `ready-for-agent` label — they are fully specified and agent-grabbable by construction.
...
**Frontier query** — the next tickets `/implement` can grab: `glab issue list -F json` scoped to open `ready-for-agent` issues, drop any with an open blocker or an assignee.
